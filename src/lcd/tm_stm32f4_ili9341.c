...
unsigned char TM_ILI9341_SendData(uint8_t data) {
  unsigned char ret;
	ILI9341_WRX_SET;
	ILI9341_CS_RESET;
	ret = TM_SPI_Send(ILI9341_SPI, data);
	ILI9341_CS_SET;
        return ret;
}
	...
	uint32_t i, b, j;
        ...
        if(c==' '){
          ...
          TM_ILI9341_SendCommand(ILI9341_MAC);
            ...
            TM_ILI9341_SendData(0x68);
          #endif
            unsigned short tmp_color, p  = font->FontWidth*font->FontHeight;
            tmp_color = background;
            
            TM_ILI9341_SetCursorPosition(x, y, x+font->FontWidth-1 ,y+font->FontHeight-1);
            //TM_ILI9341_SendCommand(ILI9341_GRAM);
            ILI9341_WRX_RESET;
            ILI9341_CS_RESET;
        ...
        unsigned short symbol[32];
        //memset( (unsigned char*)symbol,(unsigned char*)&font->data[c-32], 32  );
        for(int i=0; i<font->FontHeight;++i){
          symbol[i] = font->data[(c - 32) * font->FontHeight + i];
        }
...
//		b = font->data[(c - 32) * font->FontHeight + i];
		//b = font->data[(c - 32) * font->FontHeight + (font->FontHeight - i-1)];
                b = symbol[font->FontHeight - i-1];
...
void TM_ILI9341_DrawFilledRectangleFast(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint32_t color) {
  TM_ILI9341_SendCommand(ILI9341_MAC);
  #ifdef ROTATE_LCD
  TM_ILI9341_SendData(0xA8);
  #else
  ...
  int cnt = (x1-x0)*(y1-y0);
  TM_ILI9341_SetCursorPosition(x0, y0, x1-1, y1-1);
  ILI9341_WRX_RESET;
  ILI9341_CS_RESET;
  TM_SPI_Send(ILI9341_SPI, ILI9341_GRAM);
  ILI9341_WRX_SET;
...
//---------------------------------------------------------------------------------
void LineLF(int x1, int x2, int y, int color){
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  TM_ILI9341_SendData(0xA8);
#else
  ...
  TM_ILI9341_SetCursorPosition(x1, y, x2, y);
  ILI9341_WRX_RESET;
  ILI9341_CS_RESET;
  TM_SPI_Send(ILI9341_SPI, ILI9341_GRAM);
  ILI9341_WRX_SET;
...
void LineUP(int x, int y, int y2, int color){
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  ...
  TM_ILI9341_SetCursorPosition(y, x, 320, 320);
  //TM_ILI9341_SendCommand(ILI9341_GRAM);
  ILI9341_WRX_RESET;
  ILI9341_CS_RESET;
  TM_SPI_Send(ILI9341_SPI, ILI9341_GRAM);
...
void LineUP_EndWhite(int x, int y, int y2, int color){
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  TM_ILI9341_SendData(0x8C);
#else
...
void LineUP_Spectr(int x, int y, int y2, int color, int end_color){
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  TM_ILI9341_SendData(0x8C);
#else
  ...
  TM_SPI_Send(ILI9341_SPI, *(((unsigned char*)&end_color)+1));
  TM_SPI_Send(ILI9341_SPI, *(((unsigned char*)&end_color)+0));
  ILI9341_CS_SET;
...
//---------------------------------------------------------------------------------
void LineUP_Spectr2(int x, int y, int y2, unsigned short *color){
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  TM_ILI9341_SendData(0x8C);
#else
...
//---------------------------------------------------------------------------------
void LineUPclr(int x, int y, int y2,  unsigned short *color, float scale){
  int clr;
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  TM_ILI9341_SendData(0x8C);
  ...
  ILI9341_WRX_SET;
  for( int i=y; i<=y2; ++i ){
      clr = (int)((float)color[i]*scale);
      TM_SPI_Send(ILI9341_SPI, *(((unsigned char*)&clr)+1));
      TM_SPI_Send(ILI9341_SPI, *(((unsigned char*)&clr)+0));
  }
  ILI9341_CS_SET;
...
char fr_char_old[16];
int col_freq;
void Freq_Lcd( int x, int y, int freq, int color, int bg ){
  
  if( col_freq !=  color){
    memset( fr_char_old, 0, sizeof(fr_char_old) );
    col_freq = color;
  }
  char fr_char[16];
  memset( fr_char, 0, sizeof(fr_char) );
  sprintf( fr_char, "%08d", freq );
  int first_zero = 0;
  int xc = x;
  
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
  TM_ILI9341_SendData(0xEC);
#else
  TM_ILI9341_SendData(0x28);
#endif
  ...
  for( int i=0; i<5;++i )
  {
    if( (fr_char[i]!='0')||(first_zero == 1 )||(i==1) ){
      if( fr_char_old[i]!=fr_char[i] ){
        lcd_dig32x48( xc, y, fr_char[i], color, bg );
        fr_char_old[i] = fr_char[i];
      }
      first_zero = 1;
    }else{
      if( fr_char_old[i]!=fr_char[i] ){
        lcd_dig32x48( xc, y, fr_char[i], bg, bg );
        fr_char_old[i] = fr_char[i];
      }
    }
  ...
  for( int i=0; i<3;++i )
  {
    lcd_dig20x26( xc, y, fr_char[i+5], color, bg );
    xc+=20;
  }
  TM_ILI9341_SendCommand(ILI9341_MAC);
#ifdef ROTATE_LCD
...
//------------------------------------------------------------------------------
void lcd_dig20x26( int x, int y, char dig, int col, int bg ){
  if( (dig<'0')|| (dig>'9'))
    return;
  int j,k;
  unsigned long  ptr = (dig-0x30)*192;
  unsigned long data;
  y=239-y-26;
  
  
  TM_ILI9341_SetCursorPosition(x, y, x+19, y+25);
  ILI9341_WRX_RESET;
  ILI9341_CS_RESET;
  TM_SPI_Send(ILI9341_SPI, ILI9341_GRAM);
  ILI9341_WRX_SET;
  for(j=0; j<26; ++j)
  {
    *(((unsigned char*)&data)+3) = dig20x26[ptr];
    *(((unsigned char*)&data)+2) = dig20x26[ptr+1];
    *(((unsigned char*)&data)+1) = dig20x26[ptr+2];
    *(((unsigned char*)&data)+0) = dig20x26[ptr+3];
  
    for(k=0; k<20; ++k)
    {
      if( data&0x80000 ){
        TM_SPI_Send(ILI9341_SPI, col>>8);
        TM_SPI_Send(ILI9341_SPI, col);
      }
      else{
        TM_SPI_Send(ILI9341_SPI, bg>>8);
        TM_SPI_Send(ILI9341_SPI, bg);
      }
      data=data<<1;
    }
    ptr+=4;
...
//------------------------------------------------------------------------------
void lcd_dig32x48( int x, int y, char dig, int col, int bg ){
  if( (dig<'0')|| (dig>'9'))
    return;
  int j,k;
  unsigned long  ptr = (dig-0x30)*192;
  ...
  TM_ILI9341_SetCursorPosition(x, y, x+31, y+47);
  ILI9341_WRX_RESET;
  ILI9341_CS_RESET;
  TM_SPI_Send(ILI9341_SPI, ILI9341_GRAM);
  ILI9341_WRX_SET;
  for(j=0; j<48; ++j)
  {
    *(((unsigned char*)&data)+3) = dig32x48[ptr];
    *(((unsigned char*)&data)+2) = dig32x48[ptr+1];
    *(((unsigned char*)&data)+1) = dig32x48[ptr+2];
    *(((unsigned char*)&data)+0) = dig32x48[ptr+3];
...
//------------------------------------------------------------------------------
void TM_ILI9341_DrawTriangleRight( int x, int y, int col){
  for(int i=0; i<8;++i)
    LineUP( x+i,  y+i, y+14-i, col);
}void TM_ILI9341_DrawTriangleLeft( int x, int y, int col){
  for(int i=0; i<8;++i)
    LineUP( x+i,  y+(7-i), y+14-(7-i), col);
}